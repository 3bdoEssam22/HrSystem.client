...
import { AdminRequestsApiService } from '../../../core/api/admin-requests-api.service';
...
@Component({
  ...
  template: `
    <div class="queue-page">
      <div class="page-header">
        <h1>All Leave Requests (Admin Queue)</h1>
        <p class="subtitle">Review and decide requests requiring administrator approval.</p>
      </div>
      ...
      @if (isLoading()) {
        ...
        <div class="card state-card">
          ...
          <p class="empty-desc">All requests have been reviewed.</p>
        </div>
      } @else {
  ...
  styles: [`
    ...
    .modal-desc { font-size: 0.875rem; color: var(--color-ink-muted); margin-bottom: var(--space-3); }
    label { display: block; font-size: 0.8125rem; font-weight: 600; margin-bottom: var(--space-1); }
    textarea { width: 100%; padding: var(--space-2); border: 1px solid var(--color-border); border-radius: var(--radius); font-size: 0.875rem; margin-bottom: var(--space-3); box-sizing: border-box; }
    .modal-actions { display: flex; justify-content: flex-end; gap: var(--space-2); }
  `]
})
export class AdminRequestsComponent implements OnInit {
  private readonly adminApi = inject(AdminRequestsApiService);
  ...
  loadAll(): void {
    ...
    this.adminApi.getAll().subscribe({
      ...
      error: (err: AppError) => {
        this.errorMessage.set(err.detail || 'Failed to load requests.');
        this.isLoading.set(false);
      }
    });
  }
  ...
  approve(id: string): void {
    ...
    this.adminApi.approve(id).subscribe({
      next: () => {
  ...
  confirmReject(): void {
    ...
    this.adminApi.reject(req.id, { reason: this.rejectReason.trim() || null }).subscribe({
      next: () => {
